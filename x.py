...
class DCCConnectorWidget(QWidget):
...
class DCCCoolingWidget(QWidget):
...
class DCCModuleWidget(QWidget):
    def __init__(self, mmcore:CMMCorePlus, i: int) -> None:
        super().__init__()
        self._mmcore = mmcore
        self._dev = mmcore.getDeviceObject(f"DCCModule{i}")
        self._connectors = {i: DCCConnectorWidget(mmcore, self._dev, i+1) for i in range(3)}
        self._cooling = DCCCoolingWidget(mmcore, self._dev)
    ...
    def __init__(self, mmcore:CMMCorePlus):
        ...
        for i in range(1, 9):
            if mmcore.getProperty(self._hub.label, f"UseModule{i}") == "Yes":
                self._modules[i] = module_wdg = DCCModuleWidget(mmcore, i)
